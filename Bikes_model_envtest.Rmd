...
this notebook uses LSOA_newvariables_all.
...
pop <- st_read(".", "LSOA_newvariables_all2")
...
correlation_m <- cor(model_df[,c(3,4,7:16,18,20:22,24,27,29)])
#Rename columns
colnames(correlation_m) <- c("Station", "Density","Male","no Car", "1Car", "1-5km", "5-10km", "NS-SeC1-2","NS-SeC5-7","User16-45y", "more10km","Rain","Temp", "Slope", "Bike", "PublicTrans", "IMD", "Green_Area","Lane")
...
```{r}
rownames(correlation_m) <- c("Station", "Density", "Male", "no Car", "1Car", "1-5km", "5-10km","NS-SeC1-2", "NS-SeC5-7", "User16-45y","more10km", "Rain", "Temp", "Slope", "Bike", "PublicTrans","IMD", "Green Area","Lane")
```
...
```{r}
#Create the linear regression model, dependant variable perc bike
model1 <- lm('Bike  ~ Stn_bin + pr_16.45+ Density + male_prop + per_nocars +perc_1car+ pr_km1.5+ pr_more10k + rainfall+ meantemp + pr_nssec12 +Slope  + PublicTransp + IMD_Score + prop_green + prop_lane', LSOA_model)
summary(model1)
```
...
#Create the linear regression model, dependant variable perc bike
model1.log <- lm('log1p(Bike) ~ pr_16.45 + Density + male_prop + pr_nssec12 + per_nocars +perc_1car+ pr_km1.5+ pr_km5.10 + rainfall+ meantemp +Slope  + PublicTransp + prop_green + Stn_bin + prop_lane', LSOA_model)
summary(model1.log)
```
...
liv_log <- lm('log1p(Bike) ~  pr_16.45+ Density + male_prop +pr_nssec12 + per_nocars + pr_km1.5+ pr_more10k + rainfall+ meantemp +Slope  + PublicTransp + prop_green + Stn_bin +prop_lane', model_liv)

```{r}
#FINAL MODEL
#Create model for Liverpool log transformed prop bike
liv_log <- lm('log1p(Bike) ~  pr_16.45+ Density + male_prop +pr_nssec12 + per_nocars + pr_km1.5+ pr_km5.10 + rainfall+ meantemp +Slope  + PublicTransp + prop_green + Stn_bin +prop_lane', model_liv)
summary(liv_log)
```
...
pr_16.45+ Density + male_prop +pr_nssec12 + per_nocars + pr_km1.5+ pr_km5.10 + rainfall+ meantemp +Slope  + PublicTransp + prop_green + Stn_bin +prop_lane',

```{r}
#Final model
#Create model for Wirral without station binary
Wirr_model <- lm('log1p(Bike)~ pr_16.45+ Density + male_prop +pr_nssec12 + per_nocars +pr_km1.5+ pr_km5.10 + rainfall+ meantemp+ Slope + PublicTransp + prop_green + prop_lane', model_wirr)
summary(Wirr_model)
```
